...
function createCytoscape(elements) {
  ...
  // style DFG edges (frequency/performance) after layout
  try {
    const dfgEdges = cyInstance.edges().filter((e) => e.data("type") === "dfg");
    if (dfgEdges && dfgEdges.length > 0) {
      const freqs = dfgEdges.map((e) => Number(e.data("frequency") || 0));
      const perfs = dfgEdges.map((e) =>
        Number(e.data("performance_seconds") || 0)
      );
      const minPerf = Math.min(...perfs);
      const maxPerf = Math.max(...perfs);

      // helper to interpolate color from green to red based on normalized value 0..1
      const interpColor = (t) => {
        const clamp = (v) => Math.max(0, Math.min(1, v));
        const c = clamp(t);
        // green (#34D399) -> rgb(52,211,153)
        // red (#FF6B6B) -> rgb(255,107,107)
        const r = Math.round(52 + (255 - 52) * c);
        const g = Math.round(211 + (107 - 211) * c);
        const b = Math.round(153 + (107 - 153) * c);
        return `rgb(${r},${g},${b})`;
      };

      dfgEdges.forEach((edge) => {
        const f = Number(edge.data("frequency") || 0);
        const p = Number(edge.data("performance_seconds") || 0);
        // width scale: logarithmic to avoid huge width
        const width = Math.max(1, Math.log10(f + 1) * 3);

        // normalize performance (if all equal, t=0)
        let t = 0;
        if (maxPerf > minPerf) t = (p - minPerf) / (maxPerf - minPerf);
    ...
    console.warn("DFG styling error", err);
  }
